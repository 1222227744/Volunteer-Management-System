...
当前代码库对应第一次迭代 / 课程版 `MVP`：

- 已具备核心业务主链，可直接演示注册、活动、报名、签到签退、服务记录、内容审核、公告通知、反馈、捐赠、统计和审计。
- 仍存在一部分“文档已明确、代码未完全落地”或“功能已实现、规则待收口”的内容。
- 第二次迭代的目标不是重做，而是在最新文档基线上把业务闭环、权限边界、通知模型和数据治理补齐。
...
如果你打算继续把这个项目做成第二次迭代版本，建议先按 [第二次迭代范围与目标.md](./第二次迭代范围与目标.md) 统一开发边界。

本轮优先事项建议如下：

1. 修正通知与审计治理问题：广播通知改为用户级已读，审计查询改为真实筛选、分页和导出。
2. 补齐活动主流程缺口：活动状态流转约束、活动取消联动、报名与服务记录规则收紧。
3. 补齐课程版 `FR-05`：增加基础活动评价闭环，并与服务记录、积分排行衔接。
4. 收紧数据与权限边界：组织方数据可见范围、数据库约束、索引和迁移脚本。
