...
- `v4` 已启动生产化和质量增强工作，第一阶段补齐服务记录更正申请、审核、驳回、通过后修正记录和积分差额调整。
- 当前剩余工作主要集中在更高覆盖率自动化测试、真实第三方服务适配边界和标准迁移框架替换。
...
当前仓库已经进入第四次迭代开发阶段，建议优先按 [第四次迭代范围与目标.md](./docs/第四次迭代范围与目标.md) 做验收走查。

本轮剩余优先事项建议如下：

1. 继续补齐高价值自动化测试，覆盖通知、活动状态、评价、审计筛选、支付回调和迁移兼容等关键回归点。
2. 梳理邮件、短信、支付和文件存储的外部服务适配边界，保留课程版模拟实现并明确替换点。
3. 在轻量迁移治理基础上，继续评估是否切换到 Flyway 或 Liquibase。
4. 继续同步旧文档和最新概要设计，避免说明文档与代码口径再次漂移。

以下内容保留为后续扩展，而不是当前 v4 第一阶段强制范围：
